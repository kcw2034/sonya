...
def create_client(model: str) -> Any:
    """Helper to instantiate the correct client based on the model string."""
...
class AgentManager:
    ...
    def configure(self, model: str, system_prompt: str) -> None:
    ...
    async def chat_stream(self, user_message: str) -> Any:
        """Send a message to the agent and yield the streaming response chunks."""
        if not self._agent:
            self.configure(self.model, self.system_prompt)
            
        self.history.append({"role": "user", "content": user_message})
        
        # NOTE: Using the raw client stream for now until Agent has stream support.
        # Ensure we prepend the system prompt if needed, or pass instructions.
        messages = self.history.copy()
        if self._agent.instructions:
            messages.insert(0, {"role": "system", "content": self._agent.instructions})
            
        full_response = ""
        try:
            async for chunk in self._client.generate_stream(messages=messages):
                full_response += chunk
                yield chunk
        finally:
