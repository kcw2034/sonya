...
from sonya.core.agent._adapter import _get_adapter
...
class AgentRuntime:
    ...
    def _build_registry(self) -> ToolRegistry:
        ...
        from sonya.core.orchestration._handoff import (
            _make_handoff_tool,
        )
    ...
    async def run(
        ...
        if tools:
            ...
            provider = _provider_map.get(provider_name, 'openai')
        ...
        for _iteration in range(agent.max_iterations):
            ...
            for tc in parsed.tool_calls:
                if tc.name.startswith(_HANDOFF_PREFIX):
                    target_name = tc.name[len(_HANDOFF_PREFIX):]
                    # Callback: handoff
                    if self._callbacks:
                        for cb in self._callbacks:
        ...
        raise AgentError(
            agent.name,
            f'Exceeded max_iterations ({agent.max_iterations})',
        )
